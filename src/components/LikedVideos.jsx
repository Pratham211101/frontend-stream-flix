          ...
          <Link to={`/video/${video.video._id}`} key={video.video._id}>
            <div className="w-[300px] transform transition-transform duration-300 hover:scale-105">
              <div className="aspect-video rounded-xl overflow-hidden">
                <img
                  src={video.video.thumbnail || '/default-thumbnail.jpg'}
                  alt={video.video.title}
                  className="w-full h-full object-cover"
                />
              </div>
              <h2 className="text-md font-semibold line-clamp-2 mt-2">{video.video.title}</h2>
              <h3 className="text-sm text-gray-600 dark:text-gray-400">
                {video.video.owner?.fullname || 'Unknown Channel'}
              </h3>
              <p className="text-sm text-gray-500">
                {video.video.views} views • {getTimeAgo(video.video.createdAt)}
              </p>
            </div>
          </Link>
        ))}
